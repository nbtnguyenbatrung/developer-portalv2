  ...
  useEffect(() => {
      const loadData = async () => {
          ...
          try {
              ...
              const myProduct = productData.find((item: Product) => item.slug === params?.slug)

              if (!myProduct) return
              setProduct(myProduct)
              setContent(mdData)
              setDownloads(downloadData)
              const documentData = await call("get", `/api/docs/?category=${myProduct.productCode.toLowerCase()}`)
    ...
    useEffect(() => {
        const handleClickOutside = (event: React.ChangeEvent<HTMLInputElement>): any => {
                                  ...
                                  {isDropdownOpen && (
                                      <div className="absolute top-full left-0 mt-2 w-96 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                          <div className="bg-gradient-to-r from-primary to-primary text-white px-4 py-3">
                                              ...
                                              <p className="text-xs text-blue-100 mt-1">{t("selectDocsRead")}</p>
                                          </div>
